...
def build():
    ...
    callout(
        ...
        "25.361 s；一个两弹方案已被连续时间区间证书与独立几何方法共同验证，因此完成全窗口防御的最少弹药数为 2。"
    )
    ...
    rows = [
        ...
        ("坐标", "当前结果是相对坐标；缺少无人机初始点、基地基准和绝对任务时钟。"),
    ]
    ...
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run()
    shape = run.add_picture(str(FIGURE), width=Inches(6.15))
    shape._inline.docPr.set(
        "descr",
        "方案A烟幕并集几何与方案B独立接续在1、2、3枚干扰弹下的连续完整遮蔽时长对比，并标出M1最坏探测窗口。",
    )
    p.paragraph_format.keep_with_next = True
    cap = doc.add_paragraph("图 1　方案 A 的已验证能力前沿与方案 B 保守 baseline")
    cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
    cap.paragraph_format.space_after = Pt(10)
    set_font(cap.runs[0], 9.5, color=MUTED)

    heading(doc, "四、应优先报告的两弹全窗口方案", 1)
    callout(
        doc,
        "作战方案（相对量）",
        "烟幕中心 x=(0, 128.09582447) m；起爆时刻 t=(-6.76157575, 4.06443896) s；"
        "投放时刻 t=(-10.26157575, 0.56443896) s；需要覆盖的窗口为 "
        "[-4.37575009, 20.9852925306] s。整体平移时间和空间不会改变相对可行性。",
    )
    ...
    steps = [
        ...
        "统一相对坐标：令舰船圆心为 s(t)=7.71t；烟幕中心记为 c_j；所有长度统一用 m，时间统一用 s。输出中必须写清时间零点可以整体平移。",
        ...
        "检查无人机相对转场：由投放点到烟幕中心的惯性位移为 28×3.5=98 m；逐对检查投放时间差至少 1 s，并检查 28Δt 是否不小于相邻投放点距离。",
    ...
    paper_steps = [
        ...
        "两弹结果：先证明 1 枚弹的解析上限 10.376 s 小于 25.361 s，再引用两弹连续证书，从而得到“相对模型下最少 2 枚”的逻辑闭环。",
